<?php
/**
 * Apex LMS - Main Root Operational Control Terminal Matrix
 * Ref Layout Context: Figure 2.3 Structural Blueprint
 */
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['executeCreateBook'])) {
    ...
    if (!empty($bookTitle) && !empty($bookAuthor) && !empty($bookIsbn)) {
        try {
            ...
            $systemSuccessFeedback = "CRUD Matrix Modification Success: Resource logged securely into inventory storage.";
...
if (isset($_GET['action']) && $_GET['action'] === 'deleteRecord' && isset($_GET['id'])) {
    ...
    try {
        ...
        $systemSuccessFeedback = "CRUD Matrix Modification Success: Resource removed from catalog inventory tracking loops.";
    } catch (\PDOException $e) {
        $systemSuccessFeedback = "Data Constraint Exception: Asset locked in active circulation tracking. Delete operation blocked.";
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Apex LMS Admin Command Center Dashboard</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <small style="color:#008080; display:block; margin-bottom:20px;">Authenticated Admin Panel</small>
        <nav>
            <a href="index.php" class="active">Dashboard Overview</a>
            <a href="#">Manage Inventory</a>
            <a href="views/catalog.php">Circulation Matrix</a>
            <a href="login.php" style="color:#ff6b6b; margin-top:40px;">Terminate Session (Logout)</a>
        </nav>
    </div>
    ...
    <div class="main-workspace">
        <header style="display:flex; justify-content:between; align-items:center; margin-bottom:25px;">
            <div>
                <h1 style="color:#1A2B4C; margin:0;">Dashboard Overview</h1>
                <small style="color:#666;">Real-time library operations and analytics workspace tracker</small>
            </div>
            <div style="background:#1A2B4C; color:white; padding:10px 20px; border-radius:4px; font-weight:bold;">
                Librarian Account: <?php echo htmlspecialchars($_SESSION['full_name']); ?>
            </div>
        </header>
        ...
        <?php if (!empty($systemSuccessFeedback)): ?>
            <div style="background:#D4EDDA; color:#155724; padding:15px; border-radius:4px; margin-bottom:20px; border:1px solid #C3E6CB;">
                <strong>Pipeline Trace Info:</strong> <?php echo $systemSuccessFeedback; ?>
            </div>
        <?php endif; ?>
        ...
        <section class="metrics-container">
            <div class="metric-card">
                ...
                <small style="color:#008080;">Live persistence inventory count</small>
            </div>
            <div class="metric-card">
                <span style="color:#666; text-transform:uppercase; font-size:12px; font-weight:bold;">Active Loans Tracker</span>
            ...
            <div class="metric-card alert">
                ...
                <small style="color:#dc3545;">Action required immediately</small>
            </div>
        </section>

        <section class="data-table-container" style="margin-bottom: 30px;">
            <h3 style="color:#1A2B4C; margin-top:0;">CRUD Matrix Action: Log New Resource Entry</h3>
        ...
        <section class="data-table-container">
            <h3 style="color:#1A2B4C; margin-top:0;">Dynamic Library Registry Data Records</h3>
            <table>
                <thead>
                    <tr>
                        <th>Asset ID</th>
                        ...
                        <th>System Availability Status</th>
